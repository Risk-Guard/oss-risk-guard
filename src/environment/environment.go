...
type Config struct {
	...
	outputDir        string
	inputDir         string
	repoOutputDir    string
	checksOutputPath string
}

func (c *Config) SetInputDir(inputDir string) {
	c.inputDir = inputDir
}

// Returns the input directory for reading cached data. Falls back to output directory if not set.
func (c *Config) GetInputDir() string {
	if c.inputDir == "" {
		// Fall back to outputDir if not set
		return c.outputDir
	}
	return c.inputDir
}

func (c *Config) SetRepoOutputDir(repoOutputDir string) {
	c.repoOutputDir = repoOutputDir
}

// Returns the repo-specific output directory for writing repo-specific analysis.
// For local repos, this is <repo>/.risk-guard/cache/
// For remote repos or if not set, falls back to GetOutputDir (global cache).
func (c *Config) GetRepoOutputDir() string {
	if c.repoOutputDir == "" {
		return c.outputDir
	}
	return c.repoOutputDir
}

func (c *Config) GetChecksOutputPath() string {
	return c.checksOutputPath
}

func (c *Config) SetChecksOutputPath(path string) {
	c.checksOutputPath = path
}
...
func (c *Config) GetSaveToDatabase() bool {
	return c.SaveToDatabase
}

func (c *Config) SetSaveToDatabase(save bool) {
	c.SaveToDatabase = save
}
...
func (c *Config) Clone() *Config {
	return &Config{
		...
		FetchState:           c.FetchState,
		SaveToDatabase:       c.SaveToDatabase,
		outputDir:            c.outputDir,
		inputDir:             c.inputDir,
		repoOutputDir:        c.repoOutputDir,
		checksOutputPath:     c.checksOutputPath,
	}
}
