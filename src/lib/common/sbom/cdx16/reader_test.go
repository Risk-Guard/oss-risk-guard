...
func TestReader_RoundTripWithAndWithoutLocation(t *testing.T) {
	...
	bom, err := NewBuilder("source/repo", nodes, "test-tool").Build()
	...
	require.NotNil(t, loc, "lodash should have a location")
	...
	require.NotNil(t, loc, "requests should have a file-only location")
	require.NotNil(t, loc.File)
