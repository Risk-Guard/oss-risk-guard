...
// returns true, fetch nodes are read from disk instead of re-executed.
...
func (d *DAG[TInput]) executeNode(
	...
	var noFetch bool
	if p, ok := any(input).(NoFetchProvider); ok {
		noFetch = p.GetNoFetch()
	}
	startTime := time.Now()
...
func (d *DAG[TInput]) Execute(ctx context.Context, input TInput) (context.Context, TInput, error) {
	...
	// Track skipped keys for skip propagation. skippedMutex protects reads in
	// executeNode (siblings of a stage all read it for their upstream deps).
	// Writes happen only in the main goroutine after wg.Wait(), so the lock is
	// effectively only contended by reads, but executeNode still takes it.
	...
	for _, stage := range stages {
		...
		// Snapshot ctx for the stage. Workers read this snapshot only; the
		// main goroutine folds outputs into the outer ctx after wg.Wait().
		// Without this snapshot, the closure capture races on the outer ctx
		// variable (writers locked, readers in executeNode did not).
		// The Done channel still propagates outer cancellation because
		// stageCtx is an interface-value copy that shares the same chain.
		stageCtx := ctx
		...
		for _, key := range stage {
			...
			go func(k any) {
				...
				output, entry, isSkipped, skipReason, err := d.executeNode(stageCtx, input, k, skippedKeys, &skippedMutex)
				if err != nil {
					errChan <- err
					return
				}
		...
		// Fold outputs into ctx sequentially in the main goroutine. Topo-sort
		// guarantees in-stage siblings have no dependency on each other, so
		// the order here doesn't affect the next stage's correctness.
		for stageOut := range outputChan {
			if stageOut.isSkipped {
				ctx = stageOut.entry.storeInContext(ctx, stageOut.output)
				skippedKeys[stageOut.key] = stageOut.skipReason
				continue
			}
			output := stageOut.output
			if err = ApplyOverrides(ctx, output); err != nil {
				return ctx, input, err
			}
